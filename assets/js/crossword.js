...
var XW = (function(){
  ...
  function renderFull(gridEl, acrossEl, downEl, msgEl, titleEl){
    ...
    gridEl.innerHTML=''; gridEl.style.gridTemplateColumns='repeat('+n+',var(--xw-cell))';
    var num=0;
    for(r=0;r<n;r++)for(c=0;c<n;c++){
      var ch=grid[r][c]; sol[r][c]=ch;
      var cell=document.createElement('div'); cell.className='xw-cell';
      if(ch==='.'){ cell.classList.add('block'); gridEl.appendChild(cell); cells[r][c]=null; continue; }
      var isAc=(c===0||grid[r][c-1]==='.')&&(c+1<n&&grid[r][c+1]!=='.');
      var isDn=(r===0||grid[r-1][c]==='.')&&(r+1<n&&grid[r+1][c]!=='.');
      if(isAc||isDn){ num++; var s=document.createElement('span'); s.className='xw-num'; s.textContent=num; cell.appendChild(s); numMap[r+'-'+c]=num; }
