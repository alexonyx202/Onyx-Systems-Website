...
| `hooks/pre-push` | **Pre-push hook** — pre-gates every `main` push locally and refreshes the vault snapshot. Installed via `snapshot_site.py --install-hook` (drops a shim at `.git/hooks/pre-push` that execs this file, so edits apply without reinstalling). Fires only on `refs/heads/main`, skips outside the vault layout (no `Reference/` sibling). Local gates mirror CI `verify.yml`: freshness tokens, manifest-regeneration parity (read-only — working tree is restored), game links, daily-lineup harness, and the browser QA verifier (server on 8099 + puppeteer; skipped with a warning if node/puppeteer are absent — CI still gates). Any gate failure **blocks the push** (`git push --no-verify` to force). The snapshot step warns but doesn't block (`SNAPSHOT_STRICT=1` to block). |
